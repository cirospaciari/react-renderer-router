  ...
  getRenderer() {
    ...
    return new Promise(resolve => {
      ...
      const waitClosesAndRestart = () => {
        setImmediate(() => {
          if (this.renderers[active].sendedKillSignal && renderer.exitCode === null) {
            //waiting to close
            waitClosesAndRestart();
          } else if (this.renderers[active].sendedKillSignal) {
            //received signal and closed
            this.renderers[active] = this.createRenderer();
            resolve(this.renderers[active]);
          } else {
            //some one restart for me!
            resolve(this.renderers[active]);
          }
        });
      };
