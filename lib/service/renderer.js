...
module.exports = async function render(scope, params) {
  ...
  try {
    ...
    const $ = cheerio.load(scope.html); //use while because lazy imports can add more lazy imports!
    ...
    const Helmet = (route || {}).helmet || route.component.helmet || (() => /*#__PURE__*/React.createElement(Fragment, null));
    ...
    const helmet = /*#__PURE__*/React.createElement(Helmet, {
      model: model
    });
    ...
    try {
      dom_operations.forEach(operation => {
        ...
        if (operation.html) {
          ...
          operation.html = ReactDOMServer.renderToString( /*#__PURE__*/React.createElement(Component, null));
          element = $(operation.html).first();
    ...
    try {
      ...
      request.query = new URLSearchParams(request.search); //pre-execute lazy callbacks

      await Promise.all(getLazyCallbacks().filter(lazy => !lazy.component && lazy.options.ssr !== false).map(lazy => {
        return lazy.callback();
      }));
      const body = await renderAsync( /*#__PURE__*/React.createElement(App, {
        entry_state: entry_state,
        context: context,
        error500: true,
        request: request,
      ...
      const Helmet = (context.route || {}).helmet || ((context.route || {}).component || {}).helmet || (() => /*#__PURE__*/React.createElement(Fragment, null)); //use while because lazy imports can add more lazy imports!
      ...
      const header_html = await renderAsync( /*#__PURE__*/React.createElement(Helmet, {
        model: model
      }));
      const headElement = $('head');
      const container = $.load(`<head>${header_html}</head>`);
