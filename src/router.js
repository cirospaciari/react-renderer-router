...
class Router {
    ...
    async render(request, options, resolved_route) {
        options = options || {};
        const { route_index, route, params } = resolved_route || this.resolve(request.url);

        let rendered;

        if (route_index === -1) { //default 404
            //404
            rendered = {
                context: {
                    status: 404
            ...
            if (typeof interceptor === 'function') {
                rendered = await interceptor(render_request, {
                    render: async (request, options) => {
                        ...
                        return await this.renderer.render({ ...render_options, ...options, request });
                    }
                })
            } else {
                ...
                const fastify_request_handler = async (request, reply, route, route_index) => {
                    ...
                    if (rendered.context.cookies && reply.setCookie) {
                        for (let i in rendered.context.cookies) {
                            const cookie = rendered.context.cookies[i];
                            // cookie.options = { domain:string, path:string, signed:bool, expires:number }
                    ...
                    if (rendered.context.url) {
                        if (rendered.context.status != 301 && rendered.context.status != 302)
                            return reply.redirect(302, rendered.context.url);
